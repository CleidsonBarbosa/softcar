...
def abrir_formulario_servico(tree, dados=None, root_anterior=None):
    ...
    def excluir_servico_form():
        if not dados:
            return
        if not messagebox.askyesno("Excluir", "Tem certeza que deseja excluir este serviço?"):
            return
        try:
            conn = conectar()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM servicos WHERE id_servico = %s", (dados["id_servico"],))
            conn.commit()
            cursor.close()
            conn.close()
    ...
    canvas.tag_bind(btn_excluir_win, "<Button-1>", lambda e: excluir_servico_form())
...
def tela_servicos(root_anterior=None):
    ...
    btn_cadastrar_img_original = Image.open("assets/btn_servicos.png") if os.path.exists("assets/btn_servicos.png") else None
    ...
    canvas.tag_bind(btn_cadastrar_window, "<Button-1>", lambda e: abrir_formulario_servico(tree, root_anterior=root))
