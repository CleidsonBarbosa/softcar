...
def abrir_formulario_material(tree, dados=None):
    ...
    def excluir_material_form():
        if not dados:
            return
        if not messagebox.askyesno("Excluir", "Tem certeza que deseja excluir este material?"):
            return
        try:
            conn = conectar()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM estoque WHERE id_produto = %s", (dados["id_produto"],))
            conn.commit()
            cursor.close()
            conn.close()
            messagebox.showinfo("Sucesso", "Material excluído com sucesso!")
            modal.after(100, lambda: (modal.destroy(), tela_materiais()))
        except mysql.connector.Error as e:
            messagebox.showerror("Erro", f"Erro ao excluir:\n{e}")

    canvas.tag_bind(btn_excluir_win, "<Button-1>", lambda e: excluir_material_form())
