...
from service.servicos_service import (
    buscar_servicos,
    excluir_servico,
    listar_servicos,
    obter_servico,
...
class ServicosScreen(BaseScreen):
    ...
    def _criar_widgets(self):
        ...
        ctk.CTkButton(header, text="Novo Servico +", font=("Arial", 13, "bold"),
        ...
        self.tree.column("nome_servico", width=500, minwidth=250, stretch=True, anchor="w")
        self.tree.column("preco_servico", width=160, minwidth=120, stretch=False, anchor="center")
    ...
    def _buscar_servicos(self):
        ...
        for row in self.tree.get_children():
            self.tree.delete(row)
        try:
            for row in buscar_servicos(termo):
                preco = f"R$ {row[2]:.2f}" if row[2] else "-"
                self.tree.insert("", "end", values=(row[0], row[1], preco))
        except mysql.connector.Error as e:
...
class ServicoForm(ctk.CTkToplevel):
    def __init__(self, parent, dados=None, on_save=None):
        ...
        ctk.CTkLabel(main, text="Dados do Servico", font=("Arial", 22, "bold"), text_color=COR_DOURADO).pack(pady=(20, 25))
        ...
        for campo, label in zip(self.campos, self.labels):
            ...
            ctk.CTkLabel(row, text=label, font=("Arial", 14, "bold"), text_color=COR_BRANCO, width=120, anchor="e").pack(side="left")
    ...
    def _salvar(self):
        ...
        try:
            id_servico = self.dados["id_servico"] if self.dados else None
            salvar_servico({"nome_servico": nome, "preco_servico": preco_str}, id_servico)
            if self.on_save:
                self.on_save()
            self.destroy()
